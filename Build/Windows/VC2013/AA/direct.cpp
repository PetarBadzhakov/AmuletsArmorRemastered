...
{
    // clamp straight into your one chosen resolution
    if (newX < 0)               newX = 0;
    else if (newX >= SCREEN_WIDTH)  newX = SCREEN_WIDTH - 1;

    if (newY < 0)               newY = 0;
    else if (newY >= SCREEN_HEIGHT) newY = SCREEN_HEIGHT - 1;
...
{
    // warp OS cursor *directly* into your physical window,
    // which matches SCREEN_WIDTH×SCREEN_HEIGHT
